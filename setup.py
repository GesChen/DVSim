...
def main():
    ...
    try:
        ...
        print("ffmpeg is not installed or not on PATH. either install ffmpeg or add it to PATH.")
        return

    # check blender exists
    typical_blender_install = Path(r"C:\Program Files\Blender Foundation")
    if not typical_blender_install.exists():
        print('blender is not installed, or it is installed not at the typical system-wide location')
        print(f'some backend utility scripts rely on it being installed at {typical_blender_install}')
        print('either reinstall it there, or you may need to modify some source files')
    else:
        blender_target = typical_blender_install / "Blender 5.2/blender.exe"
        if not blender_target.exists():
            print('blender is installed, but the version used internally (5.2) is not.')
            print('either install 5.2, or you may need to modify some source files')
        else: print('blender 5.2.0 installed')
    ...
    else: print('pip requirements already satisfied')
